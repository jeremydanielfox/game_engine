...
public class BFS {
    public static Map<GraphNode, GraphNode> completeBFS (List<GraphNode> endNodes) {
        List<Map<GraphNode, Edge>> gradients =
                endNodes.stream().map(end -> completeBFS(end)).collect(Collectors.toList());
        ...
        graphNodeSet.forEach(node ->
                compositeGradient.put(node,
                                      gradients
                                              .stream()
                                              .map(map -> map.get(node))
                                              .max( (a, b) -> Double.compare(a.getCost(),
                                                                             b.getCost())).get()
                                              .getNeighbor()));
    ...
    public static Map<GraphNode, Edge> completeBFS (GraphNode endNode) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (Edge e : node.getEdges()) {
                ...
                if (!neighbor.visited()) {
                    gradientMap.put(neighbor,
                                    new Edge(node, gradientMap.get(node).getCost() + e.getCost()));
                    queue.add(neighbor);
