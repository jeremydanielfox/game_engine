...
public class Graph {
    ...
    public List<GraphNode> shortestPath (GraphNodeID start, List<GraphNodeID> endNodes, Heuristic h) {
        List<GraphNodeID> path =
                AStar.run(myNodeMap.get(start), endNodes.stream().map(n -> myNodeMap.get(n))
    ...
    private List<GraphNode> toNodes (List<GraphNodeID> ids) {
        return Collections.unmodifiableList(ids.stream().map(id -> myNodeMap.get(id))
